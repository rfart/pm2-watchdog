    ...
    function (err, apps) {
      ...
      apps.forEach((/*ProcessDescription*/ processDescription) => {
        if (processDescription.pm2_env.axm_options.isModule) {
          console.trace(
            `Process ${processDescription.name} - is PM2 module. Ignoring it`
          );
          return;
          // TODO not neccesary
        } // Ignore PM2 modules
        ...
        if (processDescription.pm2_env.status === "online") {
          ...
          console.trace(
            `Process ${processDescription.name} - is not online, no watdog yet`
          );
        }
      });
    }
      ...
      switch (data.event) {
        ...
        case "restart":
          ...
          if (data.process.status === "online") {
            ...
            console.debug(
              `WARNING: Unexpexpected status of process when starting: ${data.process.status}`
            );
          }
          break;
        case "stop":
